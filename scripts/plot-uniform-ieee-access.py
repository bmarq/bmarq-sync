...
def print_ci(data_a, data_b, data_c, figxlabel, Nr, Nc, subplot):
    ...
    if (subplot == 1):
        ...
        plt.text(delta - 1, -0.04, r'$\Delta$', color='red', fontsize=18)

        plt.text(-10.1, 0.39,
                 r'$E[T_{Sensors_{ON}}]=$' + str(round(mean_a, 1)) + '\n' + r'$Std[T_{Sensors_{ON}}]=$' + str(
                     round(data_std, 1)) + '\n' + r'$\Delta=$' + str(round(delta, 1)), fontsize=14)

        ax.set_ylabel('Occurrence', fontsize='16', weight='bold')
        ax.set_xlabel(r'a) $T_{Sensors_{ON}}$ (in s)', fontsize='16')  # , weight='bold')
        ...
        ax.set_xlabel(r'b) $T_{Sensors_{ON}}$ (in \% of $T_{ON}$)', fontsize='16')
        ...
        ax.set_xlabel(r'c) $\beta \cdot |\delta_{k,n}|$ (in s)', fontsize='16')
...
fig.subplots_adjust(left=0.10, bottom=0.15, right=0.99, top=0.90, wspace=0.55, hspace=0.10)
Nr = 1
Nc = 3
